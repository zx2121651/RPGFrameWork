...
为了让战斗表现更生动，框架提供了一套程序化的技能动画系统。您无需编写代码，只需在技能数据库中进行配置，即可实现不同的动画效果。

### 如何配置带动画的技能

在 `skill.cs` 的数据结构中，我们新增了两个字段：

*   **`animationType` (动画类型)**: 这是一个枚举，决定了动画的播放方式。
    *   `OnSelf`: 在技能使用者身上播放。适合增益(Buff)或自我治疗类技能。
    *   `OnTarget`: 在技能目标身上播放。适合大多数直接攻击或减益(Debuff)类技能。
    *   `Projectile`: 作为投射物。效果会从使用者处飞向目标。适合火球、冰箭等技能。

*   **`effectPrefabName` (效果预制件名称)**: 您需要播放的视觉效果的名称。这个名称必须对应您在**资源管理器** (`Prefab List`)中注册过的一个预制件(Prefab)。

### 制作流程示例

1.  **创建效果预制件**: 在Unity中，使用粒子系统或其他方法创建一个您想要的技能视觉效果（例如，一个爆炸效果）。将其保存为一个预制件。
2.  **注册预制件**: 打开您的 `Prefab List` 资源文件（位于 `Assets/Game/assets/share/`），添加一个新条目，将ID设置为一个您能记住的名称（例如 `explosion_effect`），并将您刚才创建的效果预制件拖拽到对应的字段中。
3.  **配置技能**: 打开您的 `Skill List` 资源文件，找到或创建一个您想配置的技能。
    - 将 `Animation Type` 设置为您想要的类型，例如 `OnTarget`。
    - 将 `Effect Prefab Name` 填写为您刚才注册的名称，即 `explosion_effect`。

现在，当这个技能在战斗中被使用时，`BattleManager` 就会自动加载 `explosion_effect` 预制件，并根据 `OnTarget` 的规则，在敌人的位置上将它实例化播放，从而实现技能动画。

### `GenerateQuest` (动态生成任务)

这是一个实验性的“AI”功能，展示了框架动态生成内容的能力。

*   **功能**: 自动生成一个简单的“寻找道具”任务。
*   **设置方法**:
    1.  在事件树中，创建一个节点，并将其 `Type` 设置为 `GenerateQuest`。
*   **执行逻辑**:
    1.  当此事件被触发时，`QuestGenerator` 会被调用。
    2.  它会扫描游戏中定义的所有NPC和道具。
    3.  它会随机选择一个NPC和一个合适的道具，然后创建一个新的动态任务（例如，“帮老奶奶找她的眼镜”）。
    4.  然后，它会在内存中尝试构建一个新的事件树来代表这个任务的对话和逻辑。
*   **当前限制**: 由于框架的事件系统是基于预先制作的 `ScriptableObject` 资源，在运行时动态创建并执行一个全新的、功能完整的事件树存在架构上的挑战。目前，此事件会成功生成任务数据并记录在 `GeneratedQuestManager` 中，但生成的事件本身还无法被执行。要实现完整功能，需要对事件系统的数据层进行重构。
